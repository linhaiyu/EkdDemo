myapp.controller('transducer_flowout_controller', ['$scope', '$timeout', 'transducer_service', function ($scope, $timeout, transducer_service) {
    var indicator_color = {
        grey:    '#d1d0d8',
        working: '#46ad00',
        idle:    '#ff9821',
    ...
    function update_transducer_indicator (data_ary) {
        $timeout(function () {
            // console.log('transducer flowout controller: update transducer indicators.');
            for (var i = 0; i < Flowout_Transducer_Num; i++) {
                ...
                if(data_ary[i] == 'working') {
                    ...
                    console.log('transducer flowout controller: Received Invalid transducer state: [%d] - %s', i, data_ary[i]);
                    return;
                }
    ...
    $scope.init_all_trs = function(level) {
        ...
        // }
            ...
            console.log('transducer flowout controller: Invalid Transducer level: ' + level);
            return;
        }
        ...
        //     transducer_service.reg_update_func("flow_out", update_transducer_indicator);
        // }
        if (init_flag.level1 == true && init_flag.level2 == true) {
            transducer_service.reg_update_func("flow_out", update_transducer_indicator);
        }
    };

    function transducer_flowout_init () {
        console.log("transducer flowout controller start...");
    ...
    transducer_flowout_init();

    /* Monitor controller's destroy event */
    $scope.$on("$destroy", function(){
        transducer_service.unreg_update_func("flow_out");
        console.log("transducer flowout controller on destroy");
    });
}]);
