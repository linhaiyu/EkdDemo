myapp.controller('transducer_flowtjl_controller', ['$scope', '$timeout', 'transducer_service', function ($scope, $timeout, transducer_service) {
    var indicator_color = {
        grey:    '#d1d0d8',
        working: '#46ad00',
        idle:    '#ff9821',
    ...
    function update_transducer_indicator (data_ary) {
        $timeout(function () {
            // console.log('transducer flowtjl controller: update transducer indicators.');
            for (var i = 0; i < Flowtjl_Transducer_Num; i++) {
                ...
                if(data_ary[i] == 'working') {
                    ...
                    console.log('transducer flowtjl controller: Received Invalid transducer state: [%d] - %s', i, data_ary[i]);
                    return;
                }
    ...
    $scope.init_all_trs = function(level) {
        ...
        // }
            ...
            console.log('transducer flowtjl controller: Invalid Transducer level: ' + level);
            return;
        }
        ...
        if (init_flag.level1 == true) {
            transducer_service.reg_update_func("flow_tjl", update_transducer_indicator);
        }
    };

    function transducer_flowtjl_init () {
        console.log("transducer flowtjl controller start...");
    ...
    transducer_flowtjl_init();

    /* Monitor controller's destroy event */
    $scope.$on("$destroy", function(){
        transducer_service.unreg_update_func("flow_tjl");
        console.log("transducer flowtjl controller on destroy");
    });
}]);
