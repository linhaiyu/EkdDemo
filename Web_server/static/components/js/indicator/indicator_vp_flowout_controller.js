/* global myapp */
myapp.controller('vp_flowout_controller', ['$scope', 'vp_service', function($scope, vp_service){
    ...
    function vp_flowout_controller_init() {
        console.log("vp flowout controller start...");
        $scope.$emit("TAB_PAGE_LOADED", "flowout");

        create_vp_charts();

        vp_service.reg_update_func("flow_out", update_vp_charts);

        /* Monitor controller's destroy event */
        $scope.$on("$destroy", function(){
            vp_service.unreg_update_func("flow_out");
            console.log("vp flowout controller on destroy");
        });
    }

    vp_flowout_controller_init();
