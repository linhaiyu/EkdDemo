myapp.controller("algorithm_flowtjl_controller", ['$scope', '$timeout', 'system_service', 'web_socket_service', 'algorithm_configure_service',
    ...
    $scope.confirm_setting = function() {
        ...
        algorithm_configure_service.set_lock_state("flow_tjl", true);
    };
    ...
    function update_status () {
        $timeout(function () {
            ...
            console.log('algorithm flowtjl controller: query system status FlowTjl[%s]', system_status);
            ...
            if (system_status == "start") {
                ...
                console.log('algorithm flowtjl controller:  received invalid system status - '+system_status);
            }
        }, 0);
    };
    ...
    function algorithm_flowtjl_init () {
        console.log('algorithm flowtjl controller: start...');
        $scope.configure_dis_flag = false;
        $scope.$emit("TAB_PAGE_LOADED", "flowtjl");

        /* Register callback function */
        algorithm_configure_service.reg_update_func("flow_tjl", control_update);
        algorithm_configure_service.reg_unlock_func("flow_tjl", control_unlock);
        ...
        var cur_data = algorithm_configure_service.get_configuration("flow_tjl");
    ...
    algorithm_flowtjl_init();

    /* Monitor controller's destroy event */
    $scope.$on("$destroy", function()
    {
        algorithm_configure_service.unreg_update_func("flow_tjl");
        algorithm_configure_service.unreg_unlock_func("flow_tjl");
        console.log("algorithm flowtjl controller on destroy");
    });
